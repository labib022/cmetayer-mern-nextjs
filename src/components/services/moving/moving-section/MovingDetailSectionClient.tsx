...
interface MovingDetailSectionClientProps {
  title: string;
  description: string;
...
export default function MovingDetailSectionClient({
  title,
  description,
  includedServices,
  image,
  ...
  priceAmount,
  priceUnit,
}: MovingDetailSectionClientProps) {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center w-full max-w-[1280px] mx-auto">
        <div className="flex flex-col gap-8">
          ...
          <div className="flex flex-col gap-5 pt-2">
            <h3 className="font-semibold text-[20px] text-[#08203c]">Included Services</h3>
            <ul className="flex flex-col gap-3.5">
              {includedServices.map((service) => (
                <li key={service} className="flex items-center gap-3">
                  ...
                  <span className="font-medium text-[16px] sm:text-[17px] text-[#111]">{service}</span>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div className="absolute bottom-6 left-6 right-6 backdrop-blur-md bg-white/90 rounded-[20px] p-4 sm:p-5 flex items-center justify-between border border-white/60 shadow-xl z-10">
            <div className="flex flex-col">
              ...
              <span className="font-bold text-[24px] sm:text-[28px] text-[#08203c]">
                {priceAmount}
                <span className="text-sm font-normal text-[#656565]">{priceUnit}</span>
              </span>
            </div>
